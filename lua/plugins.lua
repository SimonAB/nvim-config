...
-- Mini Starter (Dashboard-like start screen) - configure early (not deferred)
safe_setup("mini.starter", function(starter)
...
vim.defer_fn(function()
	...
	-- mini.nvim - enable only mini.surround for now
	safe_setup("mini.surround", function(mini_surround)
		mini_surround.setup()
	end)
...
map("n", "<leader>Cs", function()
	...
	-- Source all configuration files in proper order (explicit Lua)
	vim.cmd("luafile " .. config_path .. "/init.lua")
				...
				if vim.v.shell_error == 0 then
					...
					for _, plugin in ipairs(plugins) do
						if plugin.name == plugin_name and plugin.build then
							...
							local build_result =
								vim.fn.system("cd " .. vim.fn.shellescape(dir) .. " && " .. plugin.build)
							if vim.v.shell_error == 0 then
